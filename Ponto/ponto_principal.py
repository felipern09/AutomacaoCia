...
for matricula in matriculas_unicas:
    base = pd.DataFrame(list(intervalo(datainicio, datafim)), columns=['Data', 'Entrada 1', 'Saída 1', 'Entrada 2', 'Saída 2', 'Entrada 3', 'Saída 3'])
    ...
    geral = geral.pivot_table(index='Data', columns=geral.groupby('Data').cumcount() + 1, values='Hora', aggfunc='first')
    geral = geral.reset_index(level=[0])
    geral = geral.rename(columns={0: 'Data', 1: 'Entrada 1', 2: 'Saída 1', 3: 'Entrada 2', 4: 'Saída 2', 5: 'Entrada 3', 6: 'Saída 3'})
    
    geral1 = geral1.reset_index(drop=True)
    geral1 = geral1.pivot_table(index='Data', columns=geral1.groupby('Data').cumcount() + 1, values='Hora', aggfunc='first')
    geral1 = geral1.reset_index(level=[0])
    geral1 = geral1.rename(columns={0: 'Data', 1: 'Entrada 1', 2: 'Saída 1', 3: 'Entrada 2', 4: 'Saída 2', 5: 'Entrada 3', 6: 'Saída 3'})
    ...
    ponto.paragraphs[1].text = str(ponto.paragraphs[1].text).replace('#data1', dt.strftime(datainicio, '%d/%m/%Y')).replace('#data2', dt.strftime(datafim, '%d/%m/%Y'))
    ponto.paragraphs[2].text = str(ponto.paragraphs[2].text).replace('#emissao', dt.strftime(dt.today(), '%d/%m/%Y'))
    ponto.tables[0].rows[3].cells[1].paragraphs[0].text = str(ponto.tables[0].rows[3].cells[1].paragraphs[0].text).replace('#nome', planbase[str(matricula)])
    ponto.tables[0].rows[4].cells[1].paragraphs[0].text = str(ponto.tables[0].rows[4].cells[1].paragraphs[0].text).replace('#cod', str(matricula))
    ponto.tables[0].rows[5].cells[1].paragraphs[0].text = str(ponto.tables[0].rows[5].cells[1].paragraphs[0].text).replace('#mat', planmat[str(matricula)])
    ponto.tables[0].rows[6].cells[1].paragraphs[0].text = str(ponto.tables[0].rows[6].cells[1].paragraphs[0].text).replace('#func', planfunc[str(matricula)])
    ponto.tables[0].rows[7].cells[1].paragraphs[0].text = str(ponto.tables[0].rows[7].cells[1].paragraphs[0].text).replace('#depto', plandept[str(matricula)])
    ...
    ponto.save(rf'C:\Users\Felipe\PycharmProjects\AutomacaoCia\Ponto\Novo Codigo\Ponto {planbase[str(matricula)]}.docx')
    docx2pdf.convert(rf'C:\Users\Felipe\PycharmProjects\AutomacaoCia\Ponto\Novo Codigo\Ponto {planbase[str(matricula)]}.docx', rf'C:\Users\Felipe Rodrigues\PycharmProjects\AutomacaoCia\Ponto\Novo Codigo\Ponto {planbase[str(matricula)]}.pdf')
    os.remove(rf'C:\Users\Felipe\PycharmProjects\AutomacaoCia\Ponto\Novo Codigo\Ponto {planbase[str(matricula)]}.docx')
    # print(base)
    # C:\Users\Felipe Rodrigues\PycharmProjects\AutomacaoCia\Ponto\Novo Codigo\Ponto
